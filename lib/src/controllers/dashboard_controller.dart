...
class DashboardController extends ControllerMVC {
  ...
  /* Future<void> initializeFollowingVideos() async {
    if (swiperIndex2 == 0) {
      ...
      if (videoRepo.followingUsersVideoData.value.videos.length > 1) {
        initController2(1).whenComplete(() {
          lock = false;
        });
      }
    } else {
      if (swiperIndex2 > 0) {
        if (videoRepo.followingUsersVideoData.value.videos.length >= swiperIndex2) {
          initController2(swiperIndex2).whenComplete(() {
            lock = false;
            playController2(swiperIndex2);
            videoRepo.dataLoaded.value = true;
            videoRepo.dataLoaded.notifyListeners();
          });
        }
        if (videoRepo.followingUsersVideoData.value.videos.length >= (swiperIndex2 + 1)) {
          initController2(swiperIndex2 + 1).whenComplete(() {
            lock = false;
            videoRepo.dataLoaded.value = true;
            videoRepo.dataLoaded.notifyListeners();
          });
  ...
  Future<void> initializeVideos() async {
    if (swiperIndex == 0) {
      if (videoRepo.videosData.value.videos != null) {
        ...
        if (videoRepo.videosData.value.videos.length > 1) {
      ...
      if (swiperIndex > 0) {
        if (videoRepo.videosData.value.videos != null) {
          if (videoRepo.videosData.value.videos.length >= swiperIndex) {
            initController(swiperIndex).whenComplete(() {
              lock = false;
              playController(swiperIndex);
              videoRepo.dataLoaded.value = true;
              videoRepo.dataLoaded.notifyListeners();
            });
          }
          if (videoRepo.videosData.value.videos.length >= (swiperIndex + 1)) {
            initController(swiperIndex + 1).whenComplete(() {
              lock = false;
              videoRepo.dataLoaded.value = true;
              videoRepo.dataLoaded.notifyListeners();
            });
  ...
  Future<void> addComment(int videoId, context) async {
    ...
    print(userRepo.currentUser.value.toJson());
  ...
  Future<void> getEulaAgreement() async {
    try {
      userRepo.getEulaAgreement().then((value) {
        ...
        print(data['title']);
        print(scaffoldKey);
        // isVideoInitialized.addListener(() {
