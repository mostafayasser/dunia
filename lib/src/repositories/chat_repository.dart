  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonData['status'] == 'success') {
        if (page > 1) {
          chatData.value.chat.insertAll(
              0, ChatModel.fromJson(json.decode(response.body)['data']).chat);
        } else {
          chatData.value =
              ChatModel.fromJson(json.decode(response.body)['data']);
        }
        chatData.notifyListeners();
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonData['status'] == 'success') {
        ...
        if (page > 1) {
          chatHistoryData.value.chat.insertAll(
              0, ChatModel.fromJson(json.decode(response.body)['data']).chat);
        } else {
